...
#ifdef PROTO
//!initialize the board layoud, same as BOARD

void setup_UIModule()
{
	SerialDebug.println("----------------- PROTO Defined  -----------------");
	boolean buzzStatus = getBuzzStatus_StepperModule();
	...
	digitalWrite(GREEN_LED, LOW);  //turn the power light on  
...
#ifdef PROTO
    //set the buzzer light on
    digitalWrite(YELLOW_LED, onFlag?LOW:HIGH);
#endif
