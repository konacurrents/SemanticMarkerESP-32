...
////Put all the pins in an array to make them easy to work with
int _pins[]{
    27,  //BLUE end of the Blue/Yellow motor coil    RED is ground
    14,  //PINK end of the Pink/Orange motor coil
    12,  //YELLOW end of the Blue/Yellow motor coil
    13   //ORANGE end of the Pink/Orange motor coil
};
...
////Put all the pins in an array to make them easy to work with
// PROTOtype board is same as no BOARD
int _pins[]{
...
void whoAreWe() {
    ...
    if (getFeederType() == UNO)            // Uno makes 16 steps per revolution
        _targetSteps = 128;
    else if (getFeederType() == MINI)      // Mini makes 4 steps per revolution but reverses
        _targetSteps = 512;
    else if (getFeederType() == TUMBLER)   // Tumbler accepts a variable number of degrees 
        _targetSteps = (ceil) ( getPreferenceFloat_mainModule(PREFERENCE_STEPPER_ANGLE_FLOAT_SETTING) / 0.17665361523586); //converts degrees to motor cycles
        switch(_fudgeFactor) {
            case(0):
       ...
       SerialDebug.printf("STEPPER STEPS = %d\n", _targetSteps);
        ...
        _targetSteps = 2038;               // Fall thru handling *** new information from https://lastminuteengineers.com/28byj48-stepper-motor-arduino-tutorial/
